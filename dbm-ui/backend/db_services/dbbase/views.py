...
class DBBaseViewSet(viewsets.SystemViewSet):
    ...
    def query_biz_machine_attrs(self, request, *args, **kwargs):
        ...
        if data.get("cluster_id"):
            ...
            storage_machines = [si.machine for si in cluster.storageinstance_set.all() if si.machine]

            # 获取所有 proxy 相关的 machine
            proxy_machines = [pi.machine for pi in cluster.proxyinstance_set.all() if pi.machine]

            machines = storage_machines + proxy_machines
        ...
        if data["machine_attrs"]:
            ...
            if instance_role_flag:
                ...
                if data["cluster_type"][0] == ClusterType.TenDBCluster.value:
                    spider_roles = (
