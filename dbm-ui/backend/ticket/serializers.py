...
from backend.ticket.models import Flow, Ticket, TicketModifyRecord, Todo
...
class TicketSerializer(AuditedSerializer, serializers.ModelSerializer):
    ...
    def get_details_version(self, obj):
        # 内容版本号 = 已落库改单记录数，与 modify.TicketModifyHandler._current_version 保持一致
        return len(obj.modify_records.all())
...
class CheckTicketFlowConfigClusterTagRepeatSerializer(serializers.Serializer):
    bk_biz_id = serializers.IntegerField(help_text=_("业务ID"), required=False, default=PLAT_BIZ_ID)
    cluster_tags = serializers.ListSerializer(help_text=_("集群标签列表"), child=serializers.DictField())
    ticket_type = serializers.CharField(help_text=_("单据类型"))
    config_id = serializers.IntegerField(help_text=_("流程配置ID"), required=False, default=None)
...
class TicketModifyRecordSerializer(serializers.ModelSerializer):
    """改单记录(快照与差异)"""

    mode_display = serializers.SerializerMethodField(help_text=_("改单模式名称"))

    class Meta:
        model = TicketModifyRecord
        fields = [
            "id",
            "mode",
            "mode_display",
