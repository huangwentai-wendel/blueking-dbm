...
class MachineEventFilter(filters.FilterSet):
    ...
    ticket = filters.NumberFilter(field_name="ticket", label=_("单据id"))
    ...
    def filter_domain(self, queryset, name, value):
        cluster_ids = Cluster.objects.filter(immute_domain__in=value.split(",")).values_list("id", flat=True)
    ...
    class Meta:
        model = MachineEvent
        fields = ["operator", "bk_biz_id", "events", "ips", "create_at__lte", "create_at__gte", "domain", "ticket"]
...
class DirtyMachinePoolFilter(filters.FilterSet):
    ...
    class Meta:
        model = DirtyMachine
        fields = ["bk_host_ids", "ips", "city", "sub_zone", "rack_id", "device_class", "os_name", "update_at__lte",
                  "update_at__gte", "pool", "updator"]
